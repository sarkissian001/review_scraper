...
func (b *AppStoreScraper) _landingURL() string {
	landingURL := fmt.Sprintf("%s://%s/%s", b._protocol, b._landingHost, b._landingPath)
...
func (b *AppStoreScraper) _requestURL() string {
	requestURL := fmt.Sprintf("%s://%s/%s", b._protocol, b._requestHost, b._requestPath)
...
func NewAppStoreScraper(country, appName string, appID int, limit int) *AppStoreScraper {
	...
	new := &AppStoreScraper{
		...
		_requestParams: map[string]interface{}{
			"l":                   "en-GB",
			"offset":              0,
			"limit":               20, // we can get max 20 reviews at a time
			"platform":            "web",
	...
	new._requestHeaders.Set("Origin", new._protocol+"://"+new._requestHost)
